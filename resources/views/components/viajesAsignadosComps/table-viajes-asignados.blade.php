...
<table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
    ...
    <tbody>
        ...
        @foreach ($viajes as $key => $viaje)
            ...
            <tr
                class="{{ $viaje->progreso === 1 ? 'bg-red-300' : ($viaje->progreso === 2 ? 'bg-yellow-100' : 'bg-gray-200') }}">
                <th cope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">

                    @if($viaje->progresoSolicitud === 0)
                        @php
                            $dropdownId = "dropdown-{$viaje->id}";
                            $buttonId = "dropdownButton-{$viaje->id}";
                        @endphp

                        <button id="{{ $buttonId }}" data-dropdown-toggle="{{ $dropdownId }}"
                            class="text-gray-700 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:bg-gray-800 dark:hover:bg-gray-700 dark:focus:ring-blue-800"
                            type="button">
                                ...
                                @foreach ($choferes as $chofer)
                                    <form method="POST" action="/admin/viajes/{{$chofer->id}}">
                                        ...
                                        <li class="hover:bg-gray-100">
                                            <button type="submit"
                                                class="text-left block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white"
                                                style="white-space: normal;">{{$chofer->name}}</button>
                                        </li>
                                    </form>
                                @endforeach
                            </ul>
                ...
                <form id="form-viajes" method="POST" action="/admin/update">
                    ...
                    <td>
                        <input type="text" name="fecha_salida{{ $key }}"
                            style="border: none; background-color: transparent; width: 125px; text-align: center;"
                            class="px-2 py-1 text-gray-900"
                            value="{{ \Carbon\Carbon::parse($viaje->fecha_salida)->format('d/m/y') }}">
                    </td>
                    <td>
                    ...
                    <td>
                        <input type="text" name="fecha_llegada{{ $key }}"
                            style="border: none; background-color: transparent; width: 125px; text-align: center;"
                            class="px-2 py-1 text-gray-900"
                            value="{{ \Carbon\Carbon::parse($viaje->fecha_llegada)->format('d/m/y') }}">
                    </td>
                    ...
                    <td>
                        <input type="text" name="p_chasis{{ $key }}"
                            style="border: none; background-color: transparent; width: 125px; text-align: center;"
                            class="px-2 py-1 text-gray-900" value="{{ $viaje->p_chasis ?? '' }}">
                    </td>
                    <td>
                        <input type="text" name="p_batea{{ $key }}"
                            style="border: none; background-color: transparent; width: 125px; text-align: center;"
                            class="px-2 py-1 text-gray-900" value="{{ $viaje->p_batea ?? ''}}">
                    </td>
                    <td class="px-6 py-4  text-gray-900"><a href="#" data-modal-toggle="modalCombustible{{ $key }}"
                            class="font-medium text-blue-600 dark:text-blue-500 hover:underline" id="verMasLink">Ver más</a>
                    </td>
                    ...
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        <div class="p-4 md:p-5 text-center">
                            ...
                            <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">¿Está seguro que quiere
                                eliminar este viaje?</h3>

                            <form action="/admin/cancelar/{{$viaje->id}}" method="POST">
                                @csrf
                                @method('delete')
                                <button type="submit" data-modal-toggle="modalEliminar{{ $key }}"
                                    class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center me-2">
                                    Si, estoy seguro
                                </button>
                            </form>

                            <button data-modal-toggle="modalEliminar{{ $key }}" type="button"
            ...
            <!-- Main modal -->
            <div id="modalGastos{{ $key }}" data-modal-backdrop="static" tabindex="-1" aria-hidden="true"
                class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-modal md:h-full">
                <div class="relative p-4 w-full max-w-2xl h-full md:h-auto">
                    <!-- Modal content -->
                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                        ...
                        <div class="flex justify-between items-start p-5 rounded-t border-b dark:border-gray-600">
                            ...
                            <button type="button"
                                class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white"
                                data-modal-toggle="modalGastos{{ $key }}">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path fill-rule="evenodd"
                                        d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
